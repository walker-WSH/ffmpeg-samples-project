...
#define OUTPUT_BIT_RATE     96000   /* The output bit rate in bit/s */
#define OUTPUT_CHANNELS     2       /* The number of output channels */

static char* srcfile = "test.mp3";
static char* destfile = "test.aac";
    ...
    SwrContext **resample_context)
{
    ...
    */
    ...
    int error = swr_init(*resample_context);
    if (error < 0)
    {
    ...
    int *finished)
{
    ...
    uint8_t **converted_input_samples_buffer = NULL;
    int data_present = 0;
    ...
    if (data_present)
    {
        if (init_converted_samples(&converted_input_samples_buffer, output_codec_context, input_frame->nb_samples))
            goto cleanup;

        if (convert_samples((const uint8_t**)input_frame->extended_data, converted_input_samples_buffer, input_frame->nb_samples, resampler_context))
            goto cleanup;

        if (add_samples_to_fifo(fifo, converted_input_samples_buffer, input_frame->nb_samples))
            goto cleanup;
...
cleanup:
    if (converted_input_samples_buffer)
    {
        av_freep(&converted_input_samples_buffer[0]);
        free(converted_input_samples_buffer);
    }
    ...
    int *data_present, int *finished)
{
    ...
    {
        if (error == AVERROR_EOF) /* If we are at the end of the file, flush the decoder below. */
        {
            *finished = 1;
        }
        else
...
* @return Error code (0 if successful)
*/
static int init_converted_samples(uint8_t ***converted_input_samples_buffer,
    AVCodecContext *output_codec_context,
    int frame_size)
{
    int error;
    ...
    */
    if (!(*converted_input_samples_buffer = calloc(output_codec_context->channels, sizeof(**converted_input_samples_buffer))))
    {
    ...
    error = av_samples_alloc(*converted_input_samples_buffer, NULL,
        output_codec_context->channels, frame_size, output_codec_context->sample_fmt, 0);
    if (error < 0)
    {
...
/**
* @param[out] data_present  Indicates whether data has been encoded
* @return Error code (0 if successful)
*/
    ...
    int *data_present)
{
    *data_present = 0;
    ...
    if (frame)
    {
        static int64_t s_next_pts = 0;
        frame->pts = s_next_pts;
        s_next_pts += frame->nb_samples;
    }

    /* Send the audio frame stored in the temporary packet to the encoder.
    * The output audio stream encoder is used to do this. */
    int error = avcodec_send_frame(output_codec_context, frame);
    if (error == AVERROR_EOF)  /* The encoder signals that it has nothing more to encode. */
    {
        error = 0;
        //goto cleanup; // 此处还可以拿到一个包，应该接着调用avcodec_receive_packet
    }
    else if (error < 0)
    {
